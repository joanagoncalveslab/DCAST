...
def bias_vis_final(model_dict, n_neigh, vis_method='umap'):
    ...
    for fold in range(model_dict['params']['dataset']['runs']):
        ...
        for i, bias in enumerate(model_dict['params']['biases']):
            ...
            if len(model_dict['params']['biases'])==1:
                ax_tmp.set_title(f'{name2short[classid2name[1]]}({len(selected_positives)}/{sum(y_train==classes[1])}) - '
                            f'{name2short[classid2name[0]]}({len(selected_negatives)}/{sum(y_train==classes[0])})', y=-0.02)
            else:
        ...
        st.set_y(0.98)
        fig.subplots_adjust(top=0.95)
        notype_loc = config.ROOT_DIR / f'final_bias_umaps_only_supervised' / f'{model_dict["params"]["dataset"]["name"]}' / f'{model_dict["params"]["dataset"]["name"]}{bias_str}_nn{n_neigh}_run{fold}'
        #png_loc = config.RESULT_DIR / f'{os.path.basename(__file__)[:-3]}' / f'{model_dict["params"]["dataset"]["name"]}{bias_str}_run{fold}.png'
        config.ensure_dir(notype_loc)
        #plt.show()
        plt.savefig(f'{notype_loc}_nolegend.png', type='png', dpi=300, bbox_inches='tight')
        #plt.savefig(f'{notype_loc}_nolegend.pdf', dpi=300, bbox_inches='tight')
...
def visualize_umap_final():
    for ds_name in ['mnist', 'breast_cancer', 'wine_uci2', 'mushroom', 'fire', 'adult', 'spam', 'rice', 'raisin', 'pumpkin', 'pistachio']:
        ...
        bias_per_class = args.bias_size
        dataset = {'name': ds_name,'args':{}, 'order': 'train_test_unlabeled_bias_validation', 'train': 0.8, 'test': 0.2, 'val': 0.2, 'runs': 30}
        if 'drug_' in ds_name:
        ...
        for n_neigh in [100]:#[15, 100]:#, 50, 200]:
...
def visualize_umap():
    ...
    model_dict['params']['dataset'] = {'name': 'mnist', 'order': 'train_test_unlabeled_bias_validation',
                                       'train': 0.85, 'test': 0.25, 'unlabeled': 0.40, 'val': 0.1, 'runs': 30}
...
def get_multi_selected_similarities(model_dict, out_loc):
    main_folder = 'bias_histograms_only_supervised'
    notype_loc = config.ROOT_DIR / main_folder / f'{out_loc}_T'#f'{model_dict["params"]["dataset"]["name"]}' / 'bias_histograms' / f'{out_loc}.png'
    config.ensure_dir(notype_loc)
    ...
    if len(model_dict['params']['datasets'])==3:
        ...
        plt.rcParams["figure.figsize"] = (8.4,6) #(24,25)#((bias_size/best_facts[0])*6, (bias_size/best_facts[1])*6)
    ...
    plt.subplots_adjust(wspace=0.24, hspace=0.13)
    ...
    for i_ds, dataset in enumerate(model_dict['params']['datasets']):
        ...
        new_model_dict = {'models':{}, 'params':{}}
        new_model_dict['params']['bias'] = {'name': 'random', 'y': True, 'size':args.bias_size}#{'name':None}
        ...
        df_res_none_loc = config.ROOT_DIR / main_folder / f'data' / f'{none_bias_str}_{ds_str}_df.csv'
        config.ensure_dir(df_res_none_loc)
        if os.path.exists(df_res_none_loc):
            df_res_none = pd.read_csv(df_res_none_loc)
        else:
            #print(f'{dataset} distances started for random!')
            df_res_none, all_distances_none, all_classes_none = get_distances(new_model_dict)
        ...
        for i, bias in enumerate(model_dict['params']['biases']):
            ...
            df_res_loc = config.ROOT_DIR / main_folder / f'data' / f'{normal_bias_str}_{ds_str}_df.csv'
            ...
            ax_tmp.set_title(f'{bias_name_fix[bias["name"]]}', size=6, pad=0.1, family='Arial')
            ax_tmp.spines.right.set_visible(False)
            ax_tmp.spines.top.set_visible(False)
            ...
            for clas_id, clas in enumerate(df_res['selected_class'].unique()):
                biased_samples = df_res[df_res['selected_class']==clas]['score'].values
                nonbiased_samples = df_res_none[df_res_none['selected_class']==clas]['score'].values#df_res_none_tmp[df_res_none_tmp['selected_class']==clas]['score'].values
                res = ks_2samp(biased_samples, nonbiased_samples)
                res2 = mannwhitneyu(biased_samples, nonbiased_samples)
                test_res[clas] = res
                test_res2[clas] = res2
                if dataset["name"] == 'mnist':
                    ...
                    print(f'{dataset["name"]}\t{bias["name"]}\tclass {clas}: \t{res.pvalue:.2e}')
                else:
                    res_txt = f'Class {class_dict[dataset["name"]][clas][-2]}: {res.statistic:.2f}'
                    print(f'{dataset["name"]}\t{bias["name"]}\tclass {class_dict[dataset["name"]][clas][-2]}: \t{res.pvalue:.2e}')
                if res.pvalue < 0.05:
                    res_txt = f'{res_txt}*'
                ...
                #max_id = np.argmax(heights[clas_id*bins:(clas_id+1)*bins])
                nobias_max_id = np.argmax(np.histogram(nonbiased_samples, bins=bins, range=ranger)[0])
                bar_x_width = hist.patches[clas_id*bins+nobias_max_id].get_width()
                bar_x_begin = hist.patches[clas_id*bins+nobias_max_id].get_x()
                x_coord = bar_x_begin+bar_x_width/2
                y_coord = hist.patches[clas_id*bins+nobias_max_id].get_y()+hist.patches[clas_id*bins+nobias_max_id].get_height()
                
                bias_max_id = np.argmax(np.histogram(biased_samples, bins=bins, range=ranger)[0])
                bias_bar_x_begin = hist.patches[clas_id*bins+bias_max_id].get_x()
                bias_x_coord = bias_bar_x_begin+bar_x_width/2
                
                #print(f'{dataset["name"]}, {bias}, class {clas}\tRandom: {bar_x_begin:.2f} , Bias: {bias_bar_x_begin:.2f}')
                #x_axis_coord = ax_tmp.transLimits.inverted().transform((x_coord, y_coord))[0]
                axes_x_begin = ax_tmp.transLimits.transform((bar_x_begin, 1))[0]
                axes_x_end = ax_tmp.transLimits.transform((bar_x_begin+bar_x_width, 1))[0]
                data_y_coord = ax_tmp.transLimits.inverted().transform((0, 1))[1]
                ax_tmp.axvline(x_coord, c=color_palette[clas_id], linewidth=0.3) 
                ax_tmp.axhline(data_y_coord, axes_x_begin-0.005, axes_x_end+0.005, c=color_palette[clas_id], linewidth=0.5)
                #ax_tmp.axline((bar_x_begin, ax_tmp.get_ylim()[1]), (bar_x_begin+bar_x_width,ax_tmp.get_ylim()[1]), c=color_palette[clas_id], linewidth=0.3) 
...
def get_multi_selected_similarities2(model_dict, out_loc):
    main_folder = 'bias_histograms_only_supervised'
    notype_loc = config.ROOT_DIR / main_folder / f'{out_loc}_T'#f'{model_dict["params"]["dataset"]["name"]}' / 'bias_histograms' / f'{out_loc}.png'
    config.ensure_dir(notype_loc)
    ...
    #Plot settings
    plt.clf()
    plt.rcParams["figure.figsize"] = (6,2*len(model_dict['params']['datasets']))#(24,25)#((bias_size/best_facts[0])*6, (bias_size/best_facts[1])*6)
    plt.rcParams["legend.labelspacing"] = 0.05
    plt.rcParams["legend.handletextpad"] = 0
    total_columns = len(model_dict['params']['biases'])
    total_rows=len(model_dict['params']['datasets'])
    ...
    for i_ds, dataset in enumerate(model_dict['params']['datasets']):
        ...
        print(df_res_none.head())
        for i, bias in enumerate(model_dict['params']['biases']):
            ax_tmp = ax[i_ds, i]#ax[i_ds,i]#//total_rows, i%total_rows]
            bias_specific_model_dict = {'models':{}, 'params':{}}
            bias_specific_model_dict['params']['bias'] = bias.copy()
            if bias["name"]=='dirichlet':
                bias_specific_model_dict['params']['bias']['n'] = bias['n']*unique_class
            ...
            for clas_id, clas in enumerate(df_res['selected_class'].unique()):
                ...
                #res_txt = f'Class {clas}: {res.pvalue:.2e}'
                if dataset["name"] == 'mnist':
                    res_txt = f'Class {clas}: {res.statistic:.2f}'
                else:
                    res_txt = f'Class {class_dict[dataset["name"]][clas][-2]}: {res.statistic:.2f}'
                #ax_tmp.text(ax_tmp.get_xlim()[1]-ax_tmp.get_xlim()[1]/5, ax_tmp.get_ylim()[1] - ax_tmp.get_ylim()[1] * 2 / 10 - ax_tmp.get_ylim()[1] * clas_id / 20, res_txt,
                #         fontsize=16, ha='left', va='top', wrap=True, font='Arial')# - ax_tmp.get_ylim()[1]/40
                ...
                #max_id = np.argmax(heights[clas_id*bins:(clas_id+1)*bins])
                max_id = np.argmax(np.histogram(nonbiased_samples, bins=bins, range=ranger)[0])
                bar_x_width = hist.patches[clas_id*bins+max_id].get_width()
                bar_x_begin = hist.patches[clas_id*bins+max_id].get_x()
                x_coord = bar_x_begin+bar_x_width/2
                y_coord = hist.patches[clas_id*bins+max_id].get_y()+hist.patches[clas_id*bins+max_id].get_height()
                print(f'{bar_x_begin}, {bar_x_width}')
                #x_axis_coord = ax_tmp.transLimits.inverted().transform((x_coord, y_coord))[0]
                axes_x_begin = ax_tmp.transLimits.transform((bar_x_begin, 1))[0]
                axes_x_end = ax_tmp.transLimits.transform((bar_x_begin+bar_x_width, 1))[0]
                data_y_coord = ax_tmp.transLimits.inverted().transform((0, 1))[1]
    ...
    plt.margins(0,0)
    plt.savefig(f'{notype_loc}x_v.png', dpi=300, bbox_inches='tight', pad_inches = 0.02)
    plt.savefig(f'{notype_loc}x_v.pdf', dpi=300, bbox_inches='tight', pad_inches = 0.02)
    #plt.savefig(f'{notype_loc}.eps', dpi=300, bbox_inches='tight')
...
def draw_multi_distr():
    if args.ds_group == -1:
        datasets = ['adult', 'breast_cancer', 'fire', 'mnist', 'mushroom', 'pistachio', 'pumpkin', 'raisin', 'rice', 'spam']
    if args.ds_group == 0:
        datasets = ['wine_uci2', 'mushroom', 'fire', 'spam']
    elif args.ds_group == 1:
        datasets = ['breast_cancer', 'pumpkin', 'mnist', 'adult']
    elif args.ds_group == 2:
        datasets = ['raisin', 'rice', 'pistachio']

...
def none_hierarchy_multi_datasets(datasets, metric="LogLoss"):
    ...
    for dataset in datasets:
        ...
        try:
            ...
            dictre = {"SU": 'RF', "DST-None": 'BaST-RF', 'DST-10':'DBaST-10-RF', 'DST-100':'DBaST-100-RF'}#, "DST-10": 'H', "DST-100": 'P'}
        ...
        try:
            ...
            kmm_rf_df = get_res('results_kmm_rf_test_nb_imb_ss8', f'{bias["name"]}', dataset, out_kmm_rf_loc)
        ...
        try:
            ...
            dictre = {"SU": 'NN', "DST-None": 'BaST-NN', 'DST-10':'DBaST-10-NN', 'DST-100':'DBaST-100-NN'}#, "DST-10": 'H', "DST-100": 'P'}
        ...
        try:
            ...
            kmm_nn_df = get_res('results_kmm_nn_test_nb_imb_ss8', f'{bias["name"]}', dataset, out_kmm_nn_loc)
        ...
        try:
            ...
            lr_df = get_res('results_lr_test_nb_imb_ss8', f'{bias["name"]}', dataset, out_lr_loc)
...
def none_hierarchy_multi_datasets_facet(datasets, metric="LogLoss", swarmed=True):
    ...
    for dataset in datasets:
        ...
        bias_str = f'{bias["name"]}({"|".join([str(val) for key, val in bias.items() if "name" not in key])})'
        #Main RF
        dictre_rf = {"SU": 'Biased-RF', "DST-None": 'BaST-RF', 'DST-10':'DBaST-10-RF', 'DST-100':'DBaST-100-RF'}#, "DST-10": 'H', "DST-100": 'P'}
        try:
            model_rf_str = f'DST-BRRF1(th={th_rf}|kb={kb}|mi=100|vb=False|b=ratio)'
            out_rf_loc = f'{bias_str}_{ds_str}_{model_rf_str}_es=True'
            rf_df = get_res('results_test_nb_imb_ss8', f'{bias["name"]}', dataset, out_rf_loc)
        ...
        try:
            ...
            kmm_rf_df = get_res('results_kmm_rf_test_nb_imb_ss8', f'{bias["name"]}', dataset, out_kmm_rf_loc)
            kmm_rf_df = kmm_rf_df[~np.isin(kmm_rf_df['Method'],['SU'])]
            dictre_kmm_rf = {"KMM": 'KMM-RF'}#, "DST-10": 'H', "DST-100": 'P'}
            kmm_rf_df = kmm_rf_df.replace({"Method": dictre_kmm_rf})
            kmm_rf_df['Main Model'] = 'RF'
            if 'KMM-LR' not in kmm_rf_df['Method'].unique():
                ...
                kmm_rf_df = pd.concat([kmm_rf_df, df_dictionary])
            lst_to_concat.append(kmm_rf_df)
        except Exception as e1:
            print(f"Sorry, main kmm_rf coulnd't open: {e1}")
            tmp_dct = {'Method':'KMM-RF', 'Start_size': 1,  'EndSize': 1, 'Iteration': 0, 'LogLoss': 1, 'AUROC': 0.5, 'AUPRC': 0.5, 'Accuracy': 0.5, 'Main Model': 'RF'}
            df_dictionary = pd.DataFrame([tmp_dct])
            lst_to_concat.append(df_dictionary)
            #rf_df = pd.concat([rf_df, df_dictionary])
        
        #Main NN
        try:
            model_nn_str = f'DST-BRRF1(th=0.9|kb={kb}|mi=100|vb=False|b=ratio)'
            out_nn_loc = f'{bias_str}_{ds_str}_{model_nn_str}_es=True'
            nn_df = get_res('results_nn_test_nb_imb_fin_cw3_ss8', f'{bias["name"]}', dataset, out_nn_loc)
            nn_df = nn_df[~np.isin(nn_df['Method'],['DST-50', 'DST-200'])]
            dictre_nn = {"SU": 'Biased-NN', "DST-None": 'BaST-NN', 'DST-10':'DBaST-10-NN', 'DST-100':'DBaST-100-NN'}#, "DST-10": 'H', "DST-100": 'P'}
        ...
        try:
            ...
            print(lr_dbst_df.head())
            lr_dbst_df['Main Model'] = 'LR'
            print(lr_dbst_df['Method'].unique())
            for newmetname in dictre_lr_dbst.values():
                if newmetname not in lr_dbst_df['Method'].unique():
                    ...
                    lr_dbst_df = pd.concat([lr_dbst_df, df_dictionary])
            lst_to_concat.append(lr_dbst_df) 
        except Exception as e1:
            print(f"Sorry, main LR DBaST coulnd't open: {e1}")
            lr_dbst_df = pd.DataFrame()
            for newmetname in dictre_lr_dbst.values():
                tmp_dct = {'Method':newmetname, 'Start_size': 1,  'EndSize': 1, 'Iteration': 0, 'LogLoss': 1, 'AUROC': 0.5, 'AUPRC': 0.5, 'Accuracy': 0.5, 'Main Model': 'NN'}
                df_dictionary = pd.DataFrame([tmp_dct]).copy()
                lr_dbst_df = pd.concat([lr_dbst_df, df_dictionary])
        ...
        all_datasets_lst.append(a)
        #print(a.head())
    accepted_methods = ['Biased-RF', 'BaST-RF',  'DBaST-100-RF', 'KMM-RF', 'Biased-NN', 'BaST-NN', 'DBaST-100-NN', 'KMM-NN', 'Biased-LR', 'BaST-LR', 'DBaST-100-LR', 'KMM-LR', 'KDE-LR', 'RBA-LR', 'FLDA-LR', 'TCPR-LR', 'SUBA-LR']
    ...
    bp_color_dict = {'Biased-RF':'#FFFFFF', 'BaST-RF':'#6baed6', 'DBaST-10-RF':'#6baed6', 'DBaST-100-RF':'#6baed6', 'KMM-RF':'#DDAA33',
                'Biased-NN':'#FFFFFF', 'BaST-NN':'#41ab5d', 'DBaST-10-NN':'#41ab5d', 'DBaST-100-NN':'#41ab5d', 'KMM-NN':'#DDAA33',
    ...
    out_loc = res_folder / f'Multids_{bias["name"]}_{metric}_{args.val_base}_facet_h{mutids_name}_s={swarmed}_ss8_v4_lr={th_lr}.png'
    out_loc_pdf = res_folder / f'Multids_{bias["name"]}_{metric}_{args.val_base}_facet_h{mutids_name}_s={swarmed}_ss8_v4_lr={th_lr}.pdf' 
    config.ensure_dir(out_loc)
    plt.margins(0,0)

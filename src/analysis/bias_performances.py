...
def none_vs_all_multi_datasets_facet(datasets, base_m, bias_list, metric="LogLoss"):
    ...
    #bias = main_bias.copy()
    res_folder = config.ROOT_DIR / 'bias_supervised_comparisons' 
    all_datasets_lst = []
    ...
    for dataset in datasets:
        ...
        for bias_name in bias_list:# 0.6, 0.7, 0.8, 0.9]:
            ...
            if bias_name =='dirichlet':
                ...
                if dataset=='mnist':
                    bias['n']=args.bias_size*10
            model_lr_str = f'DST-BRRF1(th=0.9|kb={kb}|mi=100|vb=False|b=ratio)_es=True'
            bias_str = f'{bias["name"]}({"|".join([str(val) for key, val in bias.items() if "name" not in key])})'
    ...
    ds_sort = {"Breast Cancer":0, "Wine":1, "Mushroom":2, "MNIST":3, "Drug":4, "Adult":5, 'Spam':6}
    ...
    PROPS = {
        ...
        'flierprops': {'marker':'o', 'markerfacecolor':'None', 'markersize':0.6, 'linestyle':'none', 'markeredgewidth':0.3}
    }
    ...
    gf = sns.FacetGrid(all_ds, col='Dataset', row='Main Model', height=0.7, aspect=1.32, sharex='col', sharey='row', gridspec_kws={"wspace":0.08, 'hspace':0.25})
    ...
    for ax in gf.axes.flat:
        ...
        if not ax.get_ylabel():
            ...
            ax.tick_params(axis='y', width=0.1, length=1, pad=2)
        else:
        ...
        if not ax.get_xlabel():
            ...
            ax.tick_params(axis='x', width=0.1, length=1, pad=2)
        else:
    ...
    out_loc = res_folder / f'Multids_{base_m}_{bias["name"]}_{metric}_{args.val_base}_facet{mutids_name}_ss8.png'
    out_loc_pdf = res_folder / f'Multids_{base_m}_{bias["name"]}_{metric}_{args.val_base}_facet{mutids_name}_ss8.pdf'
    config.ensure_dir(out_loc)
...
if args.d_group==0:
    ...
    datasets = ['breast_cancer', 'wine_uci2', 'mushroom', 'mnist', 'rice', 'fire', 'adult', 'spam', 'raisin', 'pistachio', 'pumpkin']
for base_m in ['nn']:#['nn', 'rf']:

    ...
    if os.path.exists(res_loc):
        ...
        for fold in range(model_dict['params']['dataset']['runs']):
            if True:#try:
                ...
                reset_random_seeds(R_STATE+fold)
                ...
                res.append(['SU', len(y_b_train_val), len(y_b_train_val), fold, loss_su, auroc_su, auprc_su, acc_su])

                #KMM
                #kmm_weights = kmm.kernel_mean_matching(X_test, x_b_train, kern=model_dict['params']['model']['kernel'] )[:,0]
                clf_cw = ImportanceWeightedClassifier()
                if model_dict['params']['model']['val_base']:
                    kmm_cw_weights = clf_cw.iwe_kernel_mean_matching(x_b_train.copy(), X_test.copy())
                    sample_ws_kmm_cw = np.array([sample_ws[s_id]*kmm_cw_weights[s_id] for s_id in range(len(y_b_orig_train))])
                else:
                    kmm_cw_weights = clf_cw.iwe_kernel_mean_matching(x_b_train_val.copy(), X_test.copy())
                    sample_ws_kmm_cw = np.array([sample_ws[s_id]*kmm_cw_weights[s_id] for s_id in range(len(y_b_orig_train_val))])
                es2 = EarlyStopping(
                    monitor="val_loss",
                    patience=5,
                    restore_best_weights=True)
                reset_random_seeds(R_STATE+fold)
                kmm_cw_clf = NNModel(dim_list)
                if model_dict['params']['model']['val_base']:
                    kmm_cw_clf.fit(x_b_train.copy(), y_b_train.copy(), epochs=100,
                               validation_data=(x_b_val.copy(), y_b_val.copy()), verbose=0, callbacks=[es2], sample_weight=sample_ws_kmm_cw)
                else:
                    kmm_cw_clf.fit(x_b_train_val.copy(), y_b_train_val.copy(), epochs=100, verbose=0, sample_weight=sample_ws_kmm_cw)
                probs_kmm_cw_test = kmm_cw_clf.predict(X_test)
                probs_bin_kmm_cw_test = np.zeros(probs_kmm_cw_test.shape)
                probs_bin_kmm_cw_test[np.arange(probs_bin_kmm_cw_test.shape[0]), np.argmax(probs_kmm_cw_test, axis=-1)] = 1
                #preds_su_test = np.argmax(su_clf.predict(X_test), axis=-1)
                #y_bin_test = np.argmax(y_test, axis=-1)
                auroc_kmm_cw = roc_auc_score(y_test, probs_kmm_cw_test)  # [:, 1])
                auprc_kmm_cw = average_precision_score(y_test, probs_kmm_cw_test)  # [:, 1])
                acc_kmm_cw = accuracy_score(y_test, probs_bin_kmm_cw_test)
                print(acc_kmm_cw)
                loss_kmm_cw = log_loss(y_test, probs_kmm_cw_test)
                model_dict['models'][f'{fold}'][f'KMM_cw'] = {'model': kmm_cw_clf, 'loss': loss_kmm_cw, 'auroc': auroc_kmm_cw,
                                                          'auprc': auprc_kmm_cw, 'acc': acc_kmm_cw,
                                                          'sample_size': len(y_b_train_val)}
                res.append(['KMM_cw', len(y_b_train_val), len(y_b_train_val), fold, loss_kmm_cw, auroc_kmm_cw, auprc_kmm_cw, acc_kmm_cw])
                ...
                if model_dict['params']['model']['val_base']:
                    kmm_weights = clf.iwe_kernel_mean_matching(x_b_train.copy(), X_test.copy())
                    sample_ws_kmm = np.array([kmm_weights[s_id] for s_id in range(len(y_b_orig_train))])
                else:
                    kmm_weights = clf.iwe_kernel_mean_matching(x_b_train_val.copy(), X_test.copy())
                    sample_ws_kmm = np.array([kmm_weights[s_id] for s_id in range(len(y_b_orig_train_val))])
                es2 = EarlyStopping(
                    monitor="val_loss",
                    patience=5,
                    restore_best_weights=True)
                reset_random_seeds(R_STATE+fold)
                kmm_clf = NNModel(dim_list)
                if model_dict['params']['model']['val_base']:
                    kmm_clf.fit(x_b_train.copy(), y_b_train.copy(), epochs=100,
                               validation_data=(x_b_val.copy(), y_b_val.copy()), verbose=0, callbacks=[es2], sample_weight=sample_ws_kmm)
                else:
                    kmm_clf.fit(x_b_train_val.copy(), y_b_train_val.copy(), epochs=100, verbose=0, sample_weight=sample_ws_kmm)
                probs_kmm_test = kmm_clf.predict(X_test)
                probs_bin_kmm_test = np.zeros(probs_kmm_test.shape)
                probs_bin_kmm_test[np.arange(probs_bin_kmm_test.shape[0]), np.argmax(probs_kmm_test, axis=-1)] = 1
                #preds_su_test = np.argmax(su_clf.predict(X_test), axis=-1)
                #y_bin_test = np.argmax(y_test, axis=-1)
                auroc_kmm = roc_auc_score(y_test, probs_kmm_test)  # [:, 1])
                auprc_kmm = average_precision_score(y_test, probs_kmm_test)  # [:, 1])
                acc_kmm = accuracy_score(y_test, probs_bin_kmm_test)
                print(acc_kmm)
                loss_kmm = log_loss(y_test, probs_kmm_test)
                model_dict['models'][f'{fold}'][f'KMM'] = {'model': kmm_clf, 'loss': loss_kmm, 'auroc': auroc_kmm,
                                                          'auprc': auprc_kmm, 'acc': acc_kmm,
                                                          'sample_size': len(y_b_train_val)}
                res.append(['KMM', len(y_b_train_val), len(y_b_train_val), fold, loss_kmm, auroc_kmm, auprc_kmm, acc_kmm])
                
            else:#except Exception as e1:
            ...
            for kb in kb_list:  # [3, 6, 10, 20]:#[3,6,10]:
                for mi in [100]:  # [50,100]:
                    for val_base in [False]:#, True]:
                        ...
                        model_dict['params']['base_model'] = params
                        model_dict['params']['base_model']['name'] = 'BRRF1'
                        model_dict['params']['model'] = {'name': 'KMM', 'kernel': 'rbf', 'val_base': val_base}
                        model_dict['params']['model']['full_name'] = f'{model_dict["params"]["model"]["name"]}' \
                                                                     f'-{model_dict["params"]["base_model"]["name"]}'
                        bias_str = f'{model_dict["params"]["bias"]["name"]}' \
                                   f'({"|".join([str(val) for key, val in model_dict["params"]["bias"].items() if "name" not in key])})'
                        ...
                        ds_str = f'{model_dict["params"]["dataset"]["name"]}' \
                                 ...
                                 f'|{model_dict["params"]["dataset"]["unlabeled"]})'
                        model_str = f'{model_dict["params"]["model"]["full_name"]}' \
                                    f'(th={model_dict["params"]["model"]["kernel"]}' \
                        ...
                        try:
                            ...
                            ds_folder_name = '_'.join(ds_folder_name_list)
                            res_folder = 'results_kmm_nn_test_nb_imb_ss6'
                            log_loc = config.ROOT_DIR / res_folder / f'{model_dict["params"]["bias"]["name"]}' / ds_folder_name / f'{out_loc}.log'
                            change_log_path(log_loc)
                            selection_bias_trial(model_dict, res_folder, out_loc)

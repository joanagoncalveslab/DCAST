    ...
    if os.path.exists(res_loc):
        ...
        for fold, fold_res in res_dict_loc['models'].items():
            for model_name, model_res in fold_res.items():
                res.append([f'{model_name}', fold_res['SU']['sample_size'], model_res['model']['sample_size'],
        ...
        for fold in range(model_dict['params']['dataset']['runs']):
            try:
                ...
                su_clf = LogisticRegression(multi_class='auto', penalty='l2', max_iter=1000, class_weight='balanced', random_state=R_STATE+fold, C=5)
                ...
                try:
                    ...
                    print('KMM starts')
                    log('KMM starts')
                    kmm_clf = ImportanceWeightedClassifier(l2=5, iwe='kmm', loss='logistic')
                    try:
                        if model_dict['params']['model']['val_base']:
                            kmm_clf.fit(x_b_train.copy(), y_b_train.copy(),  X_test.copy())
                        else:
                            kmm_clf.fit(x_b_train_val.copy(), y_b_train_val.copy(),  X_test.copy())
                        probs_kmm_test = kmm_clf.clf.predict_proba(X_test.copy())
                        preds_kmm_test = kmm_clf.predict(X_test.copy())#rba_clf.classes[predsnp.argmax(probs_rba_test, axis=1)]#rba_clf.predict(X_test)
                    except:
                        kmm_lr_clf = LogisticRegression(multi_class='auto', penalty='l2', max_iter=1000, class_weight='balanced', random_state=R_STATE+fold, C=5)
                        if model_dict['params']['model']['val_base']:
                            kmm_weights = kmm_clf.iwe_kernel_mean_matching(x_b_train.copy(), X_test.copy())
                            kmm_lr_clf.fit(x_b_train.copy(), y_b_train.copy(), kmm_weights)
                        else:
                            kmm_weights = kmm_clf.iwe_kernel_mean_matching(x_b_train_val.copy(), X_test.copy())
                            kmm_lr_clf.fit(x_b_train_val.copy(), y_b_train_val.copy(), kmm_weights)
                        probs_kmm_test = kmm_lr_clf.predict_proba(X_test.copy())
                        preds_kmm_test = kmm_lr_clf.predict(X_test.copy())#rba_clf.classes[predsnp.argmax(probs_rba_test, axis=1)]#rba_clf.predict(X_test)
                    #psi = rba_clf.psi(X_test.copy(), rba_clf.theta.T, np.ones((X_test.shape[0], 1)), K=rba_clf.K)
                    auroc_kmm = roc_auc_score(y_test, probs_kmm_test)#[:, 1])
                    auprc_kmm = average_precision_score(y_test, probs_kmm_test)#[:, 1])
                    acc_kmm = accuracy_score(y_orig_test, preds_kmm_test)
                    loss_kmm = log_loss(y_test, probs_kmm_test)
                    model_dict['models'][f'{fold}'][f'KMM'] = {'model': kmm_clf, 'loss': loss_kmm, 'auroc': auroc_kmm, 'auprc': auprc_kmm, 'acc': acc_kmm,
                                                              'sample_size': len(y_b_train)}
                    res.append(['KMM', len(y_b_train), len(y_b_train), fold, loss_kmm, auroc_kmm, auprc_kmm, acc_kmm])
                except Exception as e2:
                    print(f"Sorry, fold {fold} with KMM couldn't run because of: {e2}")
                    log(f"Sorry, fold {fold} with KMM couldn't run because of: {e2}") 
                    
                try:
                    #KDE LR
                    print('KDE starts')
                    log('KDE starts')
                    kde_clf = ImportanceWeightedClassifier(l2=5, iwe='kde', loss='logistic')
                    try:
                        if model_dict['params']['model']['val_base']:
                            kde_clf.fit(x_b_train.copy(), y_b_train.copy(),  X_test.copy())
                        else:
                            kde_clf.fit(x_b_train_val.copy(), y_b_train_val.copy(),  X_test.copy())
                        probs_kde_test = kde_clf.clf.predict_proba(X_test.copy())
                        preds_kde_test = kde_clf.predict(X_test.copy())
                    except:
                        kde_lr_clf = LogisticRegression(multi_class='auto', penalty='l2', max_iter=1000, class_weight='balanced', random_state=R_STATE+fold, C=5)
                        if model_dict['params']['model']['val_base']:
                            kde_weights = kde_clf.iwe_kernel_densities(x_b_train.copy(), X_test.copy())
                            kde_lr_clf.fit(x_b_train.copy(), y_b_train.copy(), kde_weights)
                        else:
                            kde_weights = kde_clf.iwe_kernel_densities(x_b_train_val.copy(), X_test.copy())
                            kde_lr_clf.fit(x_b_train_val.copy(), y_b_train_val.copy(), kde_weights)
                        probs_kde_test = kde_lr_clf.predict_proba(X_test.copy())
                        preds_kde_test = kde_lr_clf.predict(X_test.copy())
                    #psi = rba_clf.psi(X_test.copy(), rba_clf.theta.T, np.ones((X_test.shape[0], 1)), K=rba_clf.K)
                    preds_kde_test = kde_clf.predict(X_test.copy())#rba_clf.classes[predsnp.argmax(probs_rba_test, axis=1)]#rba_clf.predict(X_test)
                    auroc_kde = roc_auc_score(y_test, probs_kde_test)#[:, 1])
                    auprc_kde = average_precision_score(y_test, probs_kde_test)#[:, 1])
                    acc_kde = accuracy_score(y_orig_test, preds_kde_test)
                    loss_kde = log_loss(y_test, probs_kde_test)
                    model_dict['models'][f'{fold}'][f'KDE'] = {'model': kde_clf, 'loss': loss_kde, 'auroc': auroc_kde, 'auprc': auprc_kde, 'acc': acc_kde,
                                                              'sample_size': len(y_b_train)}
                    res.append(['KDE', len(y_b_train), len(y_b_train), fold, loss_kde, auroc_kde, auprc_kde, acc_kde])
                except Exception as e2:
                    print(f"Sorry, fold {fold} with KDE couldn't run because of: {e2}")
                    log(f"Sorry, fold {fold} with KDE couldn't run because of: {e2}")

                try:
                    #RBA LR
                    print('RBA starts') 
                    log('RBA starts')
                    rba_clf = RobustBiasAwareClassifier(l2=5, max_iter=1000)
                    if model_dict['params']['model']['val_base']:
                        rba_clf.fit(x_b_train.copy(), y_b_train.copy(),  X_test.copy())
                    else:
                        rba_clf.fit(x_b_train_val.copy(), y_b_train_val.copy(),  X_test.copy())
                    psi = rba_clf.psi(X_test.copy(), rba_clf.theta.T, np.ones((X_test.shape[0], 1)), K=rba_clf.K)
                    probs_rba_test = rba_clf.posterior(psi)#rba_clf.predict_proba(X_test)
                    preds_rba_test = rba_clf.predict(X_test.copy())#rba_clf.classes[predsnp.argmax(probs_rba_test, axis=1)]#rba_clf.predict(X_test)
                    auroc_rba = roc_auc_score(y_test, probs_rba_test)#[:, 1])
                    auprc_rba = average_precision_score(y_test, probs_rba_test)#[:, 1])
                    acc_rba = accuracy_score(y_orig_test, preds_rba_test)
                    loss_rba = log_loss(y_test, probs_rba_test)
                    model_dict['models'][f'{fold}'][f'RBA'] = {'model': rba_clf, 'loss': loss_rba, 'auroc': auroc_rba, 'auprc': auprc_rba, 'acc': acc_rba,
                                                              'sample_size': len(y_b_train)}
                    res.append(['RBA', len(y_b_train), len(y_b_train), fold, loss_rba, auroc_rba, auprc_rba, acc_rba])
                except Exception as e2:
                    print(f"Sorry, fold {fold} with RBA couldn't run because of: {e2}")
                    log(f"Sorry, fold {fold} with RBA couldn't run because of: {e2}")

                try:
                    #FLDA LR
                    print('FLDA starts') 
                    log('FLDA starts')
                    flda_clf = FeatureLevelDomainAdaptiveClassifier(l2=5, max_iter=1000)
                    if model_dict['params']['model']['val_base']:
                        flda_clf.fit(x_b_train.copy(), y_b_train.copy(),  X_test.copy())
                    else:
                        flda_clf.fit(x_b_train_val.copy(), y_b_train_val.copy(),  X_test.copy())
                    #psi = flda_clf.psi(X_test.copy(), rba_clf.theta.T, np.ones((X_test.shape[0], 1)), K=rba_clf.K)
                    probs_flda_test = np.dot(X_test.copy(), flda_clf.theta)#flda_clf.posterior(psi)#rba_clf.predict_proba(X_test)
                    preds_flda_test = flda_clf.predict(X_test.copy())#rba_clf.classes[predsnp.argmax(probs_rba_test, axis=1)]#rba_clf.predict(X_test)
                    auroc_flda = roc_auc_score(y_test, probs_flda_test)#[:, 1])
                    auprc_flda = average_precision_score(y_test, probs_flda_test)#[:, 1])
                    acc_flda = accuracy_score(y_orig_test, preds_flda_test)
                    loss_flda = log_loss(y_test, probs_flda_test)
                    model_dict['models'][f'{fold}'][f'FLDA'] = {'model': flda_clf, 'loss': loss_flda, 'auroc': auroc_flda, 'auprc': auprc_flda, 'acc': acc_flda,
                                                              'sample_size': len(y_b_train)}
                    res.append(['FLDA', len(y_b_train), len(y_b_train), fold, loss_flda, auroc_flda, auprc_flda, acc_flda])
                except Exception as e2:
                    print(f"Sorry, fold {fold} with FLDA couldn't run because of: {e2}")
                    log(f"Sorry, fold {fold} with FLDA couldn't run because of: {e2}")

                try:
                    #TCPR LR
                    print('TCPR starts') 
                    log('TCPR starts')
                    tcpr_clf = TargetContrastivePessimisticClassifier(l2=1, max_iter=1000)
                    if model_dict['params']['model']['val_base']:
                        tcpr_clf.fit(x_b_train.copy(), y_b_train.copy(),  X_test.copy())
                    else:
                        tcpr_clf.fit(x_b_train_val.copy(), y_b_train_val.copy(),  X_test.copy())
                    probs_tcpr_test = tcpr_clf.predict_proba(X_test.copy())
                    preds_tcpr_test = tcpr_clf.predict(X_test.copy())#rba_clf.classes[predsnp.argmax(probs_rba_test, axis=1)]#rba_clf.predict(X_test)
                    auroc_tcpr = roc_auc_score(y_test, probs_tcpr_test)#[:, 1])
                    auprc_tcpr = average_precision_score(y_test, probs_tcpr_test)#[:, 1])
                    acc_tcpr = accuracy_score(y_orig_test, preds_tcpr_test)
                    loss_tcpr = log_loss(y_test, probs_tcpr_test)
                    model_dict['models'][f'{fold}'][f'TCPR'] = {'model': tcpr_clf, 'loss': loss_tcpr, 'auroc': auroc_tcpr, 'auprc': auprc_tcpr, 'acc': acc_tcpr,
                                                              'sample_size': len(y_b_train)}
                    res.append(['TCPR', len(y_b_train), len(y_b_train), fold, loss_tcpr, auroc_tcpr, auprc_tcpr, acc_tcpr])
                except Exception as e2:
                    print(f"Sorry, fold {fold} with TCPR couldn't run because of: {e2}")
                    log(f"Sorry, fold {fold} with TCPR couldn't run because of: {e2}")

                try:
                    #SUBA LR
                    print('SUBA starts') 
                    log('SUBA starts')
                    suba_clf = SubspaceAlignedClassifier(l2_regularization=5)
                    if model_dict['params']['model']['val_base']:
                        suba_clf.fit(x_b_train.copy(), y_b_train.copy(),  X_test.copy())#, X_unk.copy())
                    else:
                        suba_clf.fit(x_b_train_val.copy(), y_b_train_val.copy(),  X_test.copy())#, X_unk.copy())
                    #psi = rba_clf.psi(X_test.copy(), rba_clf.theta.T, np.ones((X_test.shape[0], 1)), K=rba_clf.K)
                    probs_suba_test = suba_clf.predict_proba(X_test.copy())
                    preds_suba_test = suba_clf.predict(X_test.copy())#rba_clf.classes[predsnp.argmax(probs_rba_test, axis=1)]#rba_clf.predict(X_test)
                    auroc_suba = roc_auc_score(y_test, probs_suba_test)#[:, 1])
                    auprc_suba = average_precision_score(y_test, probs_suba_test)#[:, 1])
                    acc_suba = accuracy_score(y_orig_test, preds_suba_test)
                    loss_suba = log_loss(y_test, probs_suba_test)
                    model_dict['models'][f'{fold}'][f'SUBA'] = {'model': suba_clf, 'loss': loss_suba, 'auroc': auroc_suba, 'auprc': auprc_suba, 'acc': acc_suba,
                                                              'sample_size': len(y_b_train)}
                    res.append(['SUBA', len(y_b_train), len(y_b_train), fold, loss_suba, auroc_suba, auprc_suba, acc_suba])
                except Exception as e2:
                    print(f"Sorry, fold {fold} with SUBA couldn't run because of: {e2}")
                    log(f"Sorry, fold {fold} with SUBA couldn't run because of: {e2}")
                ...
                for mi in [100]:
                    for val_base in [False]:#, True]:
                        ...
                        try:
                            ...
                            [ds_folder_name_list.append(str(val)) for val in model_dict["params"]["dataset"]["args"].values()]
                            ds_folder_name = '_'.join(ds_folder_name_list)
                            res_fold = 'results_lr_test_nb_imb_ss6'

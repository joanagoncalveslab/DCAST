...
#from sklearn.semi_supervised import SelfTrainingClassifier
from src.models.vanilla_st import SelfTrainingClassifier
K.clear_session()
...
def selection_bias_trial(model_dict, out_loc):
    ...
    res_folder = config.ROOT_DIR / 'results_nn_extra_test_nb_imb_fin_cw3_ss' / f'{model_dict["params"]["bias"]["name"]}' / ds_folder_name
    ...
    if os.path.exists(res_loc):
        ...
        for fold, fold_res in res_dict_loc['models'].items():
            for model_name, model_res in fold_res.items():
                res.append([f'{model_name}', fold_res['SU']['sample_size'], model_res['model'].labeled_sample_size,
                            fold, model_res['model']['loss'], model_res['model']['auroc'], model_res['model']['auprc'],
                            model_res['model']['acc']])
        res_df = pd.DataFrame(res, columns=['Method', 'Start_size', 'EndSize', 'Iteration', 'LogLoss', 'AUROC', 'AUPRC',
                                            'Accuracy'])
        ...
        for fold in range(model_dict['params']['dataset']['runs']):
            ...
            y_b_train = encoder.fit_transform(y_b_train.reshape(-1,1)).toarray()
            y_b_val = encoder.transform(y_b_val.reshape(-1,1)).toarray()
            y_test = encoder.transform(y_test.reshape(-1,1)).toarray()
            nnconfig.input_dim = x_b_train.shape[1]
            ...
            print(sample_ws.shape)
            print(y_b_train.copy().shape)
            np.random.seed(R_STATE)
            if model_dict['params']['model']['val_base']:
                ...
                su_clf.fit(x_b_train.copy(), y_b_train.copy(), epochs=100, verbose=0, sample_weight=sample_ws)
            ...
            model_dict['models'][f'{fold}'][f'SU'] = {'model': su_clf, 'loss': loss_su, 'auroc': auroc_su,
            ...
            try:
                #ST with Threshold
                model_th_params = {key: val for key, val in model_dict['params']['model'].items() if 'name' not in key}
                base_th_clf = NNModel(dim_list)
                np.random.seed(R_STATE+fold)
                st_th_clf = SelfTrainingClassifier(base_th_clf, criterion='threshold', threshold=model_dict['params']['model']['threshold'], 
                                                   verbose=False, max_iter=model_dict['params']['model']['max_iter'])
                st_th_clf.fit(x_b_tr_unk.copy(), y_b_tr_unk.copy(), encoder=encoder)
                end_size_th = sum(st_th_clf.labeled_iter_>=0)
                print(f'Number of samples at the end for ST with th:{end_size_th}')
                log(f'Number of samples at the end for ST with th:{end_size_th}')
                probs_st_th_test = st_th_clf.predict_proba(X_test)
                probs_bin_st_th_test = np.zeros(probs_st_th_test.shape)
                probs_bin_st_th_test[np.arange(probs_bin_st_th_test.shape[0]), np.argmax(probs_st_th_test, axis=-1)] = 1
                #preds_st_test = st_clf.predict(X_test)
                auroc_st_th = roc_auc_score(y_test, probs_st_th_test)  # [:, 1])
                auprc_st_th = average_precision_score(y_test, probs_st_th_test)  # [:, 1])
                acc_st_th = accuracy_score(y_test, probs_bin_st_th_test)
                #acc_st = accuracy_score(y_bin_test, preds_st_test)
                loss_st_th = log_loss(y_test, probs_st_th_test)
                model_dict['models'][f'{fold}'][f'ST_th'] = {'model': model_dict, 'loss': loss_st_th, 'auroc': auroc_st_th, 
                                                             'auprc': auprc_st_th, 'acc': acc_st_th, 'end_sample_size': end_size_th}
                res.append([f'ST_th', len(y_b_train), end_size_th, fold, loss_st_th, auroc_st_th, auprc_st_th,acc_st_th])
            except Exception as e4:
                print(f"Sorry, ST with th coulnd't run on fold-{fold} due to: {e4}")
                log(f"Sorry, ST with th coulnd't run on fold-{fold} due to: {e4}")

            try:
                #ST with Kbest
                model_kb_params = {key: val for key, val in model_dict['params']['model'].items() if 'name' not in key}
                base_kb_clf = NNModel(dim_list)
                np.random.seed(R_STATE+fold)
                st_kb_clf = SelfTrainingClassifier(base_kb_clf, criterion='k_best', k_best=model_dict['params']['model']['k_best'], 
                                                   verbose=False, max_iter=model_dict['params']['model']['max_iter'])
                st_kb_clf.fit(x_b_tr_unk.copy(), y_b_tr_unk.copy(), encoder=encoder)
                end_size_kb = sum(st_kb_clf.labeled_iter_>=0)
                print(f'Number of samples at the end for ST with kbest:{end_size_kb}')
                log(f'Number of samples at the end for ST with kbest:{end_size_kb}')
                probs_st_kb_test = st_kb_clf.predict_proba(X_test)
                probs_bin_st_kb_test = np.zeros(probs_st_kb_test.shape)
                probs_bin_st_kb_test[np.arange(probs_bin_st_kb_test.shape[0]), np.argmax(probs_st_kb_test, axis=-1)] = 1
                #preds_st_test = st_clf.predict(X_test)
                auroc_st_kb = roc_auc_score(y_test, probs_st_kb_test)  # [:, 1])
                auprc_st_kb = average_precision_score(y_test, probs_st_kb_test)  # [:, 1])
                acc_st_kb = accuracy_score(y_test, probs_bin_st_kb_test)
                #acc_st = accuracy_score(y_bin_test, preds_st_test)
                loss_st_kb = log_loss(y_test, probs_st_kb_test)
                model_dict['models'][f'{fold}'][f'ST_kb'] = {'model': model_dict, 'loss': loss_st_kb, 'auroc': auroc_st_kb, 
                                                             'auprc': auprc_st_kb,'acc': acc_st_kb, 'end_sample_size': end_size_kb}
                res.append([f'ST_kb', len(y_b_train), end_size_kb, fold, loss_st_kb, auroc_st_kb, auprc_st_kb, acc_st_kb])
            except Exception as e3:
                print(f"Sorry, ST with kbest coulnd't run on fold-{fold} due to: {e3}")
                log(f"Sorry, ST with kbest coulnd't run on fold-{fold} due to: {e3}")
            '''
                ...
                for mi in [100]:  # [50,100]:
                    for val_base in [False]:#, True]:
                        ...
                        try:
                            ...
                            ds_folder_name = '_'.join(ds_folder_name_list)
                            log_loc = config.ROOT_DIR / 'results_nn_extra_test_nb_imb_fin_cw3_ss' / f'{model_dict["params"]["bias"]["name"]}' / ds_folder_name / f'{out_loc}.log'

...
def selection_bias_trial(model_dict, out_loc):
    ...
    res_folder = config.ROOT_DIR / 'results_val_bias_imb' / f'{model_dict["params"]["bias"]["name"]}' / ds_folder_name
        ...
        for fold in range(model_dict['params']['dataset']['runs']):
            ...
            if model_dict['params']['bias']["name"] is not None:
                ...
                if 'y' in model_dict['params']['bias'] and model_dict['params']['bias']['y']:
                    selected_ids = bt.get_bias(model_dict['params']['bias']['name'], X_all, y=y_all,
                                               **bias_params).astype(int)
                else:
                    selected_ids = bt.get_bias(model_dict['params']['bias']['name'], X=X_all, **bias_params).astype(int)
                mask = np.ones_like(y_all, bool)
                mask[selected_ids] = False
                X_b_all, y_b_all = X_all[selected_ids, :], y_all[selected_ids]
            else:
                X_b_train, y_b_train = X_train.copy(), y_train.copy()
            if model_dict['params']['dataset']['name'] in ['ppi', 'cora', 'citeseer', 'webkb']:
                X_b_all = X_b_all[:,2:]
            b_splits = split_dataset(X_b_all, y_b_all,
                                                                   train_ratio=model_dict['params']['dataset']['train'], 
                                                                   test_ratio = model_dict['params']['dataset']['test'],
                                                                   unknown_ratio = model_dict['params']['dataset']['val'], 
                                                                   r_seed=R_STATE + fold)
            x_b_train, y_b_train = b_splits['x_train'], b_splits['y_train']
            X_test, y_test = b_splits['x_test'], b_splits['y_test']
            x_b_val, y_b_val = b_splits['x_unk'], b_splits['y_unk']
            
            zv_cols = np.var(x_b_train, axis=0)==0 #zero variance columns
            x_b_train, x_b_val, X_b_all, X_test, X_unk = \
                x_b_train[:, ~zv_cols], x_b_val[:, ~zv_cols], X_b_all[:, ~zv_cols], X_test[:, ~zv_cols], X_unk[:, ~zv_cols]
            ...
            try:
                ...
                for diversity in [None, 10, 50, 100, 200]: #3,5,10,None
                ...
                warnings.warn(f"Sorry, Bias didn't work for run {fold}: {e}", UserWarning)
                log(f"Sorry, Bias didn't work for run {fold}: {e}")
                continue
            '''
...
if args.dataset == 'cifar':
    ...
    dataset = {'name': args.dataset, 'args':{}, 'order': 'train_test_unlabeled_bias_validation',
                                                       'train': 0.7, 'test': 0.2, 'unlabeled': 0.40, 'val': 0.1, 'runs': 30}
elif 'ppi_' in args.dataset:
    dataset = {'name': 'ppi','args':{'id': int(args.dataset.split('_')[1])}, 'order': 'train_test_unlabeled_bias_validation',
                                                       'train': 0.2, 'test': 0.25, 'unlabeled': 0.55, 'val': 0.1, 'runs': 30}
elif args.dataset in ['cora', 'citeseer']:
    dataset = {'name': args.dataset, 'args':{'chosen_class': 'Neural_Networks'}, 'order': 'train_test_unlabeled_bias_validation',
                                                       'train': 0.2, 'test': 0.25, 'unlabeled': 0.55, 'val': 0.1, 'runs': 30}
elif 'webkb_' in args.dataset:
    if len(args.dataset.split('_')) == 2:
        dataset = {'name': 'webkb','args':{'source': args.dataset.split('_')[1], 'chosen_class': 'all', 'pca':None}, 'order': 'train_test_unlabeled_bias_validation',
                                                       'train': 0.7, 'test': 0.2, 'unlabeled': 0.40, 'val': 0.1, 'runs': 30}
    if len(args.dataset.split('_')) == 3:
        dataset = {'name': 'webkb','args':{'source': args.dataset.split('_')[1], 'chosen_class': 'all', 'pca':int(args.dataset.split('_')[2])}, 'order': 'train_test_unlabeled_bias_validation',
                                                       'train': 0.7, 'test': 0.2, 'unlabeled': 0.40, 'val': 0.1, 'runs': 30} 
else:
    dataset = {'name': args.dataset,'args':{}, 'order': 'train_test_unlabeled_bias_validation',
                                                       'train': 0.15, 'test': 0.75, 'unlabeled': 0.40, 'val': 0.1, 'runs': 30}
bias_list = []
...
if args.bias == 'entity':
    bias_list.append({'name': 'entity', 'y': True, 'chosen_entity_size': 8, 'max_size': bias_per_class, 'prob': 0.8, 'dominant_class':1}) 
if args.bias == 'none':
...
for bias in bias_list:
    for th in [85, 97]:#[85, 93, 97, 0.7, 0.8]:
        for balance in ['ratio','free']:#['ratio', 'free', 'equal']:
                ...
                for mi in [100]:#[50,100]:
                    for val_base in [False, True]:
                        ...
                        try:
                            ...
                            [ds_folder_name_list.append(str(val)) for val in model_dict["params"]["dataset"]["args"].values()]
                            ds_folder_name = '_'.join(ds_folder_name_list)
                            log_loc = config.ROOT_DIR / 'results_val_bias_imb' / f'{model_dict["params"]["bias"]["name"]}' / ds_folder_name / f'{out_loc}.log'

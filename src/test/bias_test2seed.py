        ...
        for fold, fold_res in res_dict_loc['models'].items():
            for seed_run, seed_res in fold_res.items():
                for model_name, model_res in seed_res.items():
                    try:
                        res.append([f'{model_name}', seed_res['SU']['sample_size'], model_res['model'].labeled_sample_size,
        ...
        for fold in range(model_dict['params']['dataset']['runs']):
            try:
                ...
                for seed_run in range(10):
                    ...
                    model_dict['models'][f'{fold}'][f'{seed_run}'][f'SU'] = {'model': None, 'loss': loss_su, 'auroc': auroc_su, 'auprc': auprc_su, 'acc': acc_su,
                    ...
                    for diversity in [None, 10, 100]: #3,5,10,None
                        try:   
                            np.random.seed(R_STATE+fold+seed_run)
                            base_clf = lgb.LGBMClassifier(boosting_type="rf", **base_m_params, random_state=R_STATE+fold+seed_run)
                            st_clf = FreeDiverseSelfTraining(base_clf, diverse=diversity, verbose=False, **model_params)
                            if args.early_stop:
                                st_clf.fit(x_b_tr_val_unk.copy(), y_b_tr_val_unk.copy(), x_b_val.copy(), y_b_val.copy())
                            else:
                                st_clf.fit(x_b_tr_val_unk.copy(), y_b_tr_val_unk.copy())
                            ...
                            model_dict['models'][f'{fold}'][f'{seed_run}'][f'ST_{diversity}'] = {'model': None, 'loss': loss_st, 'auroc': auroc_st, 'auprc': auprc_st, 'acc': acc_st,
                                                                                 'end_sample_size': st_clf.labeled_sample_size}
                            res.append([f'DST-{diversity}', len(y_b_train), st_clf.labeled_sample_size, fold, seed_run, loss_st, auroc_st, auprc_st, acc_st])
                ...
                for mi in [100]:
                    for val_base in [False]:#, True]:
                        ...
                        try:
                            ...
                            [ds_folder_name_list.append(str(val)) for val in model_dict["params"]["dataset"]["args"].values()]
                            ds_folder_name = '_'.join(ds_folder_name_list)
                            res_fold = 'results_test_nb_imb_ss8'

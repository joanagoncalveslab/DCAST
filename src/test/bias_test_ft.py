...
param_grid_skopt = {
    ...
    "reg_lambda": (5, 10),
    "min_split_gain": (0, 10),
    "subsample": (0.7, 0.9), "subsample_freq": [1],
...
def selection_bias_trial(model_dict, out_loc):
    ...
    res_folder = config.ROOT_DIR / 'results_test_nb_ft' / f'{model_dict["params"]["bias"]["name"]}' / ds_folder_name
    ...
    if os.path.exists(res_loc):
        ...
        for fold in range(model_dict['params']['dataset']['runs']):
            try:
                ...
                gs_clf = lgb.LGBMClassifier(boosting_type="rf")
                mycv = StratifiedKFold(n_splits=10, shuffle=True, random_state=R_STATE).split(X=x_b_train.copy(), y=y_b_train.copy())
                opt = BayesSearchCV(
                    gs_clf,
                    param_grid_skopt,
                    n_iter=100,
                ...
                opt.fit(X=x_b_train.copy(), y_b_train.copy())
                best_params = opt.best_params_
                print(best_params)
                print(lgb_model.best_score_)
                
                #su_clf = lgb.LGBMClassifier(boosting_type="rf", **base_m_params)
                su_clf = lgb.LGBMClassifier(boosting_type="rf", **best_params)
                ...
                for diversity in [None, 10, 50, 100, 200]: #3,5,10,None
                    base_clf = lgb.LGBMClassifier(boosting_type="rf", **best_params)
                ...
                warnings.warn(f"Sorry, something didn't work for run {fold}: {ef}", UserWarning)
                log(f"Sorry, something didn't work for run {fold}: {ef}")
                continue
            '''
                ...
                for mi in [100]:
                    for val_base in [False, True]:
                        ...
                        try:
                            ...
                            [ds_folder_name_list.append(str(val)) for val in model_dict["params"]["dataset"]["args"].values()]
                            ds_folder_name = '_'.join(ds_folder_name_list)
                            log_loc = res_folder = config.ROOT_DIR / 'results_test_nb_imb_ft' / f'{model_dict["params"]["bias"]["name"]}' / ds_folder_name / f'{out_loc}.log'

...
def selection_bias_trial(model_dict, out_loc):
    ...
    res_folder = config.ROOT_DIR / 'results_kmm_nn_test_nb_imb_ss' / f'{model_dict["params"]["bias"]["name"]}' / ds_folder_name
    ...
    if os.path.exists(res_loc):
        ...
        for fold in range(model_dict['params']['dataset']['runs']):
            try:
                ...
                sample_ws_kmm = np.array([sample_ws[s_id]*kmm_weights[s_id] for s_id in range(len(y_b_orig_train))])
                es2 = EarlyStopping(
                    monitor="val_loss",
                    patience=5,
                    restore_best_weights=True)
                ...
                if model_dict['params']['model']['val_base']:
                    kmm_clf.fit(x_b_train.copy(), y_b_train.copy(), epochs=100,
                               validation_data=(x_b_val.copy(), y_b_val.copy()), verbose=0, callbacks=[es], sample_weight=sample_ws_kmm)
                else:
                    kmm_clf.fit(x_b_train.copy(), y_b_train.copy(), epochs=100, verbose=0, sample_weight=sample_ws_kmm)
                ...
                for mi in [100]:  # [50,100]:
                    for val_base in [False, True]:
                        ...
                        try:
                            ...
                            ds_folder_name = '_'.join(ds_folder_name_list)
                            log_loc = config.ROOT_DIR / 'results_kmm_nn_test_nb_imb_ss' / f'{model_dict["params"]["bias"]["name"]}' / ds_folder_name / f'{out_loc}.log'

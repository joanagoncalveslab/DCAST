        ...
        for fold in range(model_dict['params']['dataset']['runs']):
            ...
            try:
                if model_dict['params']['bias']["name"] is not None:
                    ...
                    print(f"{model_dict['params']['bias']['name']} applied")
                    log(f"{model_dict['params']['bias']['name']} applied")
            ...
            '''
        if len(res)==0:
            raise Exception(f"Sorry, all the AUROC score for start is too low. Last one: {acc_su}")
...
if args.dataset == 'cifar':
    ...
    dataset = {'name': args.dataset, 'args':{}, 'order': 'train_test_unlabeled_bias_validation',
                                                       'train': 0.35, 'test': 0.25, 'unlabeled': 0.40, 'val': 0.2, 'runs': 30}
    ...
    dataset = {'name': 'drug','args':{'drug': args.dataset.split('_')[1]}, 'order': 'train_test_unlabeled_bias_validation',
                                                       'train': 0.3, 'test': 0.25, 'unlabeled': 0.45, 'val': 0.2, 'runs': 30}
...
if args.bias == 'entity7':
    bias_list.append({'name': 'entity', 'y': True, 'chosen_entity_size': 8, 'max_size': bias_per_class, 'prob': 0.7, 'dominant_class':1}) 
...
if args.bias == 'entity9':
    bias_list.append({'name': 'entity', 'y': True, 'chosen_entity_size': 8, 'max_size': bias_per_class, 'prob': 0.9, 'dominant_class':1}) 
if args.bias == 'entity_elisl':
    bias_list.append({'name': 'entity_elisl', 'y': True, 'chosen_entity_size': 5, 'max_size': bias_per_class, 'prob': 0.7, 'cancer':'BRCA'}) 
if args.bias == 'none':
                ...
                for mi in [100]:#[50,100]:
                    for val_base in [False, True]:
                        ...
                        try:
                            ...
                            [ds_folder_name_list.append(str(val)) for val in model_dict["params"]["dataset"]["args"].values()]
                            ds_folder_name = '_'.join(ds_folder_name_list)
                            log_loc = config.ROOT_DIR / 'results_val_nb_imb' / f'{model_dict["params"]["bias"]["name"]}' / ds_folder_name / f'{out_loc}.log'

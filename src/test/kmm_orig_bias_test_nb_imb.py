...
def selection_bias_trial(model_dict, out_loc):
    ...
    res_folder = config.ROOT_DIR / 'results_test_nb_imb' / f'{model_dict["params"]["bias"]["name"]}' / ds_folder_name
                ...
                for mi in [100]:
                    for val_base in [False, True]:
                        ...
                        try:
                            ...
                            [ds_folder_name_list.append(str(val)) for val in model_dict["params"]["dataset"]["args"].values()]
                            ds_folder_name = '_'.join(ds_folder_name_list)
                            log_loc = res_folder = config.ROOT_DIR / 'results_test_nb_imb' / f'{model_dict["params"]["bias"]["name"]}' / ds_folder_name / f'{out_loc}.log'

...
def selection_bias_trial(model_dict, out_loc):
    ...
    res_folder = config.ROOT_DIR / 'results_val_nb_bal' / f'{model_dict["params"]["bias"]["name"]}' / ds_folder_name
    ...
    if os.path.exists(res_loc):
        ...
        for fold in range(model_dict['params']['dataset']['runs']):
            ...
            if model_dict['params']['dataset']['name'] in ['ppi', 'cora', 'citeseer', 'webkb']:
                X_imb_test, y_imb_test = p_data['x_test'][:,2:], p_data['y_test']
                X_unk, y_unk = p_data['x_unk'][:,2:], p_data['y_unk']
            else:
                X_imb_test, y_imb_test = p_data['x_test'], p_data['y_test']
                X_unk, y_unk = p_data['x_unk'], p_data['y_unk']
            t_classes, t_class_counts = np.unique(y_imb_test, return_counts=True)
            sampling_dict = dict(zip(t_classes, np.repeat(min(t_class_counts), len(t_classes))))
            X_test, y_test = make_imbalance(X_imb_test, y_imb_test, sampling_strategy=sampling_dict, random_state=R_STATE + fold)
...
for bias in bias_list:
    for th in [85, 97]:#[85, 93, 97, 0.7, 0.8]:
        for balance in ['free','ratio']:#['ratio', 'free', 'equal']:
                ...
                for mi in [100]:#[50,100]:
                    for val_base in [False, True]:
                        ...
                        try:
                            ...
                            [ds_folder_name_list.append(str(val)) for val in model_dict["params"]["dataset"]["args"].values()]
                            ds_folder_name = '_'.join(ds_folder_name_list)
                            log_loc = config.ROOT_DIR / 'results_val_nb_bal' / f'{model_dict["params"]["bias"]["name"]}' / ds_folder_name / f'{out_loc}.log'

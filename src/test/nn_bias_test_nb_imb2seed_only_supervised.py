...
parser.add_argument('--fold_start', '-fs', metavar='the-fold-start', dest='fold_start', type=int,
                    help='Choose the bias size per class', default=0)
...
def selection_bias_trial(model_dict, res_folder, out_loc):
    ...
    if os.path.exists(res_loc):
        ...
        for fold in range(*model_dict['params']['dataset']['runs']):
            ...
            except Exception as ebias:
                warnings.warn(f"Sorry, Bias didn't work for run {fold}", UserWarning)
                log(f"Sorry, Bias didn't work for run {fold}: {ebias}")
                continue
            ...
            x_b_train, x_b_val, X_b_train, X_test = \
                x_b_train[:, ~zv_cols], x_b_val[:, ~zv_cols], X_b_train[:, ~zv_cols], X_test[:, ~zv_cols]
...
if args.dataset == 'cifar':
    ...
    dataset = {'name': args.dataset, 'args': {}, 'order': 'train_test_unlabeled_bias_validation',
               'train': 0.8, 'test': 0.2, 'val': 0.2, 'runs': (args.fold_start,max(args.fold_start+5, 30))}
...
for bias in bias_list:
    for th in [0.9]:  # [85, 93, 97, 0.7, 0.8]:
            ...
            for kb in kb_list:  # [3, 6, 10, 20]:#[3,6,10]:
                for mi in [100]:  # [50,100]:
                    for val_base in [False]:#, True]:
                        ...
                        out_loc = f'{bias_str}_{ds_str}_{model_str}_es={args.early_stop}_{model_dict["params"]["dataset"]["runs"]}'
                        try:
                            ds_folder_name_list = [model_dict["params"]["dataset"]["name"]]
                            ...
                            ##ss8: Same but we have 10 runs inside 30 runs and seeds are: 123+fold+run
                            res_fold = 'results_nn_test_nb_imb_fin_cw3_ss8' 

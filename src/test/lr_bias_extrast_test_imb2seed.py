...
from src.models.lr_fsd_self_training import FreeDiverseSelfTraining
        ...
        for fold in range(model_dict['params']['dataset']['runs']):
            try:
                ...
                for seed_run in range(10):
                    ...
                    try:
                        #ST with Threshold
                        base_th_clf = LogisticRegression(multi_class='auto', penalty='l2', max_iter=1000, class_weight='balanced', random_state=R_STATE+fold+seed_run, C=5)
                        np.random.seed(R_STATE+fold+seed_run)
                    ...
                    try:
                        #ST with Kbest
                        base_kb_clf = LogisticRegression(multi_class='auto', penalty='l2', max_iter=1000, class_weight='balanced', random_state=R_STATE+fold+seed_run, C=5)
                        np.random.seed(R_STATE+fold+seed_run)
                ...
                for mi in [100]:
                    for val_base in [False]:#, True]:
                        ...
                        try:
                            ...
                            [ds_folder_name_list.append(str(val)) for val in model_dict["params"]["dataset"]["args"].values()]
                            ds_folder_name = '_'.join(ds_folder_name_list)
                            res_fold = 'results_lr_extra_test_nb_imb_ss8'

        ...
        for fold in range(model_dict['params']['dataset']['runs']):
            try:
                ...
                for seed_run in range(10):
                    ...
                    base_m_params_kmm = {key: val for key, val in model_dict['params']['base_model'].items() if 'name' not in key}
                    #weights = kmm.kernel_mean_matching(X_test, x_b_train, kern=model_dict['params']['model']['kernel'] )[:,0]
                    np.random.seed(R_STATE+fold+seed_run)
                    clf = ImportanceWeightedClassifier()
                    if model_dict['params']['model']['val_base']:
                        weights = clf.iwe_kernel_mean_matching(x_b_train.copy(), X_test.copy())
                        kmm_clf = lgb.LGBMClassifier(boosting_type="rf", **base_m_params, random_state=R_STATE+fold+seed_run)
                        kmm_clf.fit(x_b_train.copy(), y_b_train.copy(), eval_set=[(x_b_val.copy(), y_b_val.copy())], eval_metric="binary_logloss", callbacks=[lgb.early_stopping(100, verbose=False)], sample_weight=weights)
                    else:
                        weights = clf.iwe_kernel_mean_matching(x_b_train_val.copy(), X_test.copy())
                        kmm_clf = lgb.LGBMClassifier(boosting_type="rf", **base_m_params, random_state=R_STATE+fold+seed_run)
                        kmm_clf.fit(x_b_train_val.copy(), y_b_train_val.copy(), sample_weight=weights)
                    probs_kmm_test = kmm_clf.predict_proba(X_test, n_jobs=-1)
                    preds_kmm_test = kmm_clf.predict(X_test, n_jobs=-1)
                    ...
                    res.append(['KMM', len(y_b_train_val), len(y_b_train), fold, seed_run, loss_kmm, auroc_kmm, auprc_kmm, acc_kmm])
            ...
            '''
        if len(res)==0:
            raise Exception(f"Sorry, the AUROC score for start is too low: {acc_su}")
            log(f'Sorry, all the AUROC score for start is too low. Last one: {acc_su}')
                ...
                for mi in [100]:
                    for val_base in [False]:#, True]:
                        ...
                        try:
                            ...
                            [ds_folder_name_list.append(str(val)) for val in model_dict["params"]["dataset"]["args"].values()]
                            ds_folder_name = '_'.join(ds_folder_name_list)
                            res_folder = 'results_kmm_rf_test_nb_imb_ss8'

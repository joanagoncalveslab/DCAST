...
def selection_bias_trial(model_dict, out_loc):
    ...
    res_folder = config.ROOT_DIR / 'results_nn_val_nb_imb' / f'{model_dict["params"]["bias"]["name"]}' / ds_folder_name
        ...
        for fold in range(model_dict['params']['dataset']['runs']):
            ...
            try:
                if model_dict['params']['bias']["name"] is not None:
                    ...
                    if 'y' in model_dict['params']['bias'] and model_dict['params']['bias']['y']:
                        ...
                        selected_ids = bt.get_bias(model_dict['params']['bias']['name'], X=X_train,
                                                   **bias_params).astype(int)
            ...
            if model_dict['params']['model']['val_base']:
                su_clf.fit(x_b_train.copy(), y_b_train.copy(), epochs=100,
                           validation_data=(x_b_val.copy(), y_b_val.copy()), verbose=0, callbacks=[es])
            else:
                su_clf.fit(x_b_train.copy(), y_b_train.copy(), epochs=100, verbose=0)
            ...
            for diversity in [None, 10, 100]:  # 3,5,10,None
                ...
                st_clf = NNDiverseSelfTraining(base_clf, diverse=diversity, verbose=False, **model_params)
...
for bias in bias_list:
    for th in [0.9, 85, 97]:  # [85, 93, 97, 0.7, 0.8]:
                ...
                for mi in [100]:  # [50,100]:
                    for val_base in [False, True]:
                        ...
                        try:
                            ...
                            ds_folder_name = '_'.join(ds_folder_name_list)
                            log_loc = config.ROOT_DIR / 'results_nn_val_nb_imb' / f'{model_dict["params"]["bias"]["name"]}' / ds_folder_name / f'{out_loc}.log'

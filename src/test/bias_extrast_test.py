...
def selection_bias_trial(model_dict, out_loc):
    ...
    res_folder = config.ROOT_DIR / 'results_extra_test_nb_imb_ss' / f'{model_dict["params"]["bias"]["name"]}' / ds_folder_name
        ...
        for fold in range(model_dict['params']['dataset']['runs']):
            try:
                ...
                try:
                    ...
                    base_th_clf = lgb.LGBMClassifier(boosting_type="rf", **base_m_params)
                    ...
                    st_th_clf.fit(x_b_tr_unk.copy(), y_b_tr_unk.copy())
                    ...
                    preds_st_th_test = st_th_clf.predict(X_test)
                    auroc_st_th = roc_auc_score(y_test, probs_st_th_test)  # [:, 1])
                    auprc_st_th = average_precision_score(y_test, probs_st_th_test)  # [:, 1])
                    #print(f'{y_test.shape},{preds_st_th_test.shape}')
                    acc_st_th = accuracy_score(y_orig_test, preds_st_th_test)
                ...
                try:
                    ...
                    base_kb_clf = lgb.LGBMClassifier(boosting_type="rf", **base_m_params)
                    np.random.seed(R_STATE+fold)
                    st_kb_clf = SelfTrainingClassifier(base_kb_clf, criterion='k_best', k_best=model_dict['params']['model']['k_best'], 
                                                       verbose=False, max_iter=model_dict['params']['model']['max_iter'])
                    st_kb_clf.fit(x_b_tr_unk.copy(), y_b_tr_unk.copy())
                    ...
                    preds_st_kb_test = st_kb_clf.predict(X_test)
                    ...
                    acc_st_kb = accuracy_score(y_orig_test, preds_st_kb_test)
                ...
                for mi in [100]:
                    for val_base in [False]:#, True]:
                        ...
                        try:
                            ...
                            [ds_folder_name_list.append(str(val)) for val in model_dict["params"]["dataset"]["args"].values()]
                            ds_folder_name = '_'.join(ds_folder_name_list)
                            log_loc = res_folder = config.ROOT_DIR / 'results_extra_test_nb_imb_ss' / f'{model_dict["params"]["bias"]["name"]}' / ds_folder_name / f'{out_loc}.log'

R_STATE = 123
import os
os.environ['PYTHONHASHSEED']=str(R_STATE)
import sys
...
def selection_bias_trial(model_dict, out_loc):
    ...
    res_folder = config.ROOT_DIR / 'results_nn_test_nb_imb_fin_cw3_ss2_sigmo' / f'{model_dict["params"]["bias"]["name"]}' / ds_folder_name
    ...
    if os.path.exists(res_loc):
        ...
        for fold in range(model_dict['params']['dataset']['runs']):
            ...
            for diversity in [None, 10, 100]:#50, 100, 200]:  # 3,5,10,None
                try:
                    ...
                    #np.random.seed(R_STATE+fold)
                    st_clf = NNDiverseSelfTraining(base_clf, diverse=diversity, verbose=False, r_seed = R_STATE+fold, **model_params)
                    if args.early_stop:
                        st_clf.fit(x_b_tr_unk.copy(), y_b_tr_unk.copy(), x_b_val.copy(), y_b_val.copy(), output_encoder=encoder)
                    else:
                        st_clf.fit(x_b_tr_unk.copy(), y_b_tr_unk.copy(), output_encoder=encoder)
                    ...
                    model_dict['models'][f'{fold}'][f'ST_{diversity}'] = {'model': model_dict, 'loss': loss_st,
                                                                          'auroc': auroc_st, 'auprc': auprc_st,
                                                                          'acc': acc_st,
                                                                          'end_sample_size': st_clf.labeled_sample_size}
                    res.append(
                        [f'DST-{diversity}', len(y_b_train), st_clf.labeled_sample_size, fold, loss_st, auroc_st, auprc_st,
                         acc_st])
                ...
                for mi in [100]:  # [50,100]:
                    for val_base in [False, True]:
                        ...
                        try:
                            ...
                            ds_folder_name = '_'.join(ds_folder_name_list)
                            #cw4 = classweight balanced and ss = seeds are fixed for sure. 
                            log_loc = config.ROOT_DIR / 'results_nn_test_nb_imb_fin_cw3_ss2_sigmo' / f'{model_dict["params"]["bias"]["name"]}' / ds_folder_name / f'{out_loc}.log'

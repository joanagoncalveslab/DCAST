...
def selection_bias_trial(model_dict, out_loc):
    ...
    res_folder = config.ROOT_DIR / 'results_kmm_test_nb_imb' / f'{model_dict["params"]["bias"]["name"]}' / ds_folder_name
    ...
    if os.path.exists(res_loc):
        ...
        res_df = pd.DataFrame(res, columns=['Method', 'Start_size', 'Iteration', 'LogLoss', 'AUROC', 'AUPRC', 'Accuracy'])
        ...
        for fold in range(model_dict['params']['dataset']['runs']):
            ...
            if np.unique(y_b_train).shape[0]>2:
            ...
            if model_dict['params']['model']['val_base']:
                su_clf.fit(x_b_train.copy(), y_b_train.copy(), eval_set=[(x_b_val.copy(), y_b_val.copy())], eval_metric=eval_metric, callbacks=[lgb.early_stopping(100, verbose=False)])
            else:
            ...
            res.append(['SU', len(y_b_train), fold, loss_su, auroc_su, auprc_su, acc_su])
            ...
            if model_dict['params']['model']['val_base']:
                kmm_clf.fit(x_b_train.copy(), y_b_train.copy(), eval_set=[(x_b_val.copy(), y_b_val.copy())], eval_metric=eval_metric, callbacks=[lgb.early_stopping(100, verbose=False)], sample_weight=weights)
            else:
            ...
            probs_st_test = kmm_clf.predict_proba(X_test)
            preds_st_test = kmm_clf.predict(X_test)
            ...
            model_dict['models'][f'{fold}'][f'KMM'] = {'model': kmm_clf, 'loss': loss_st, 'auroc': auroc_st, 'auprc': auprc_st, 'acc': acc_st}
            res.append([f'KMM', len(y_b_train), fold, loss_st, auroc_st, auprc_st, acc_st])
            '''
...
for bias in bias_list:
    for kernel in ['rbf']:
        for val_base in [False, True]:
            ...
            model_dict['params']['base_model'] = params
            model_dict['params']['base_model']['name'] = 'RRF1'
            ...
            ds_str = f'{model_dict["params"]["dataset"]["name"]}' \
                     ...
                     f'|{model_dict["params"]["dataset"]["unlabeled"]})'
            model_str = f'{model_dict["params"]["model"]["full_name"]}' \
                        f'|k={model_dict["params"]["model"]["kernel"]}'\
            ...
            try:
                ...
                [ds_folder_name_list.append(str(val)) for val in model_dict["params"]["dataset"]["args"].values()]
                ds_folder_name = '_'.join(ds_folder_name_list)
                log_loc = res_folder = config.ROOT_DIR / 'results_kmm_test_nb_imb' / f'{model_dict["params"]["bias"]["name"]}' / ds_folder_name / f'{out_loc}.log'

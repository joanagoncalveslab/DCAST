                ...
                for seed_run in range(10):
                    ...
                    try:
                        ...
                        try:
                            ...
                            kmm_lr_clf = LogisticRegression(multi_class='auto', penalty='l2', max_iter=1000, class_weight='balanced', random_state=R_STATE+fold+seed_run, C=5)
                            if model_dict['params']['model']['val_base']:
                        ...
                        model_dict['models'][f'{fold}'][f'{seed_run}'][f'KMM'] = {'model': kmm_clf, 'loss': loss_kmm, 'auroc': auroc_kmm, 'auprc': auprc_kmm, 'acc': acc_kmm,
                                                                  'sample_size': len(y_b_train)}
                        res.append(['KMM', len(y_b_train), len(y_b_train), fold, seed_run, loss_kmm, auroc_kmm, auprc_kmm, acc_kmm])
                    ...
                    try:
                        ...
                        try:
                            ...
                            kde_lr_clf = LogisticRegression(multi_class='auto', penalty='l2', max_iter=1000, class_weight='balanced', random_state=R_STATE+fold+seed_run, C=5)
                            if model_dict['params']['model']['val_base']:
                        ...
                        model_dict['models'][f'{fold}'][f'{seed_run}'][f'KDE'] = {'model': kde_clf, 'loss': loss_kde, 'auroc': auroc_kde, 'auprc': auprc_kde, 'acc': acc_kde,
                                                                  'sample_size': len(y_b_train)}
                        res.append(['KDE', len(y_b_train), len(y_b_train), fold, seed_run, loss_kde, auroc_kde, auprc_kde, acc_kde])
                    except Exception as e2:
                    ...
                    try:
                        ...
                        model_dict['models'][f'{fold}'][f'{seed_run}'][f'RBA'] = {'model': rba_clf, 'loss': loss_rba, 'auroc': auroc_rba, 'auprc': auprc_rba, 'acc': acc_rba,
                                                                  'sample_size': len(y_b_train)}
                        res.append(['RBA', len(y_b_train), len(y_b_train), fold, seed_run, loss_rba, auroc_rba, auprc_rba, acc_rba])
                    except Exception as e2:
                    ...
                    try:
                        ...
                        model_dict['models'][f'{fold}'][f'{seed_run}'][f'FLDA'] = {'model': flda_clf, 'loss': loss_flda, 'auroc': auroc_flda, 'auprc': auprc_flda, 'acc': acc_flda,
                                                                  'sample_size': len(y_b_train)}
                        res.append(['FLDA', len(y_b_train), len(y_b_train), fold, seed_run, loss_flda, auroc_flda, auprc_flda, acc_flda])
                    except Exception as e2:
                    ...
                    try:
                        ...
                        log('TCPR starts')
                        np.random.seed(R_STATE+fold+seed_run)
                        tcpr_clf = TargetContrastivePessimisticClassifier(l2=1, max_iter=100)
                        if model_dict['params']['model']['val_base']:
                        ...
                        model_dict['models'][f'{fold}'][f'{seed_run}'][f'TCPR'] = {'model': tcpr_clf, 'loss': loss_tcpr, 'auroc': auroc_tcpr, 'auprc': auprc_tcpr, 'acc': acc_tcpr,
                                                                  'sample_size': len(y_b_train)}
                        res.append(['TCPR', len(y_b_train), len(y_b_train), fold, seed_run, loss_tcpr, auroc_tcpr, auprc_tcpr, acc_tcpr])
                    except Exception as e2:
                    ...
                    try:
                        ...
                        model_dict['models'][f'{fold}'][f'{seed_run}'][f'SUBA'] = {'model': suba_clf, 'loss': loss_suba, 'auroc': auroc_suba, 'auprc': auprc_suba, 'acc': acc_suba,
                                                                  'sample_size': len(y_b_train)}
                        res.append(['SUBA', len(y_b_train), len(y_b_train), fold, seed_run, loss_suba, auroc_suba, auprc_suba, acc_suba])
                    except Exception as e2:
                ...
                for mi in [100]:
                    for val_base in [False]:#, True]:
                        ...
                        try:
                            ...
                            [ds_folder_name_list.append(str(val)) for val in model_dict["params"]["dataset"]["args"].values()]
                            ds_folder_name = '_'.join(ds_folder_name_list)
                            res_fold = 'results_lr_test_nb_imb_ss8'

...
def selection_bias_trial(model_dict, out_loc):
    ...
    res_folder = config.ROOT_DIR / 'results_lr_fsd_test_nb_imb_ss' / f'{model_dict["params"]["bias"]["name"]}' / ds_folder_name
        ...
        for fold in range(model_dict['params']['dataset']['runs']):
            try:
                ...
                for diversity in [None, 10, 100]: #3,5,10,None
                    try:
                        base_clf = LogisticRegression(multi_class='auto', penalty='l2', max_iter=1000, class_weight='balanced', random_state=R_STATE, C=5)
                ...
                for mi in [100]:
                    for val_base in [False]:#, True]:
                        ...
                        try:
                            ...
                            [ds_folder_name_list.append(str(val)) for val in model_dict["params"]["dataset"]["args"].values()]
                            ds_folder_name = '_'.join(ds_folder_name_list)
                            log_loc = res_folder = config.ROOT_DIR / 'results_lr_fsd_test_nb_imb_ss' / f'{model_dict["params"]["bias"]["name"]}' / ds_folder_name / f'{out_loc}.log'

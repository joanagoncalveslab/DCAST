...
class SelfTrainingClassifier(MetaEstimatorMixin, BaseEstimator):
    ...
    def fit(self, X, y, encoder=None):
        ...
        try:
            ...
            self.base_estimator_.compile(loss=str(self.base_estimator.loss.name),
                                         optimizer=str(self.base_estimator.optimizer._name),
                                         metrics=self.base_estimator.compiled_metrics._metrics.copy())
        ...
        while not np.all(has_label) and (
            ...
            if encoder is not None:
                self.base_estimator_.fit(
                    X[safe_mask(X, has_label)], encoder.transform(self.transduction_[has_label].reshape(-1,1)).toarray()
                )
            else:
                self.base_estimator_.fit(
            ...
            if encoder is not None:
                pred = encoder.categories_[0][np.argmax(prob, axis=1)]
            else:

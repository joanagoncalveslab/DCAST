    ...
    def fit(self, X, y, X_val=None, y_val=None):
        ...
        while not np.all(has_label) and (
            ...
            if self.val_loss_dict_ is not None:
                ...
                val_prob = self.base_estimator_.predict_proba(X_val)
            ...
            if self.threshold is not None and self.threshold > 1:
                ...
                for class_id in self.base_estimator_.classes_:
                    probs_tr_by_class = self.base_estimator_.predict_proba(X[safe_mask(X, has_label)])[:,np.where(self.base_estimator_.classes_==class_id)[0][0]]
            ...
            if self.balance == 'free':
                ...
                for label in self.base_estimator_.classes_:
                    ...
                    if self.threshold is not None and self.threshold > 1:
                        probs_of_class = self.base_estimator_.predict_proba(X[safe_mask(X, has_label)])[:,np.where(self.base_estimator_.classes_==label)[0][0]]

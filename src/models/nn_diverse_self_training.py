...
class NNDiverseSelfTraining:
    ...
    def __init__(self, base_estimator, threshold=None, k_best=None, max_iter=10, balance='equal', diverse=None,
                 verbose=False, val_base=False, r_seed=123):
        ...
        #t_random.set_seed(123)

    @timeit
    def fit(self, X, y, X_val=None, y_val=None, output_encoder=None, lossfn=BinaryCrossentropy()):
        """
        ...
        self.base_estimator_ = clone_model(self.base_estimator)
        #self.base_estimator_.compile(loss=CategoricalCrossentropy(), optimizer='adam', metrics=['accuracy'])
        self.base_estimator_.compile(loss=lossfn, optimizer='adam', metrics=['accuracy'])
        self.estimator_list = {}
        self.history_list = {}
        ...
        label_dict = {A: B for A, B in enumerate(output_encoder.categories_[0])}
        keras_classes = np.unique(y[has_label])
        ...
        self.n_iter_ = 0
        start_prob_y = output_encoder.transform(self.transduction_[has_label].reshape(-1,1)).toarray()
        # y = np.copy(y)  # copy in order not to change original data
        # all_labeled = False
        class_weights = class_weight.compute_class_weight(class_weight = "balanced", classes = np.unique(self.transduction_[has_label].copy()), y = self.transduction_[has_label].copy())
        class_weights_dict = dict(zip(np.unique(self.transduction_[has_label].copy()), class_weights))
        sample_ws = np.array([class_weights_dict[lab] for lab in self.transduction_[has_label]])
        if self.val_loss_dict_ is not None and self.val_base:
            history = self.base_estimator_.fit(
                X[safe_mask(X, has_label)],
                output_encoder.transform(self.transduction_[has_label].reshape(-1,1)).toarray(),
                validation_data=(X_val, y_val), epochs = 100, verbose=self.keras_verbose, sample_weight=sample_ws)
            self.estimator_list[self.n_iter_] = self.base_estimator_
            self.history_list[self.n_iter_] = history
        else:
            history = self.base_estimator_.fit(
                X[safe_mask(X, has_label)],
                output_encoder.transform(self.transduction_[has_label].reshape(-1,1)).toarray(), epochs = 100, verbose=self.keras_verbose, sample_weight=sample_ws
            )
            self.estimator_list[self.n_iter_] = self.base_estimator_
            self.history_list[self.n_iter_] = history
        ...
        while not np.all(has_label) and (
            ...
            if self.val_loss_dict_ is not None:
                ...
                val_prob = self.base_estimator_.predict(X_val)
            ...
            if self.threshold is not None and self.threshold > 1:
                thold_per_class = []
                for class_id in range(start_prob_y.shape[1]):#self.base_estimator_.classes_:
                    probs_tr_by_class = self.base_estimator_.predict(X[safe_mask(X, has_label)])[:,class_id]
                    thold_per_class.append(np.percentile(probs_tr_by_class, self.threshold))
                threshold_ = max(thold_per_class)
            # Predict on the unlabeled samples
            prob = self.base_estimator_.predict(X[safe_mask(X, ~has_label)])
            pred = np.argmax(prob, axis=-1)
            max_proba = np.max(prob, axis=1)
            selected = np.array([])
            if self.balance == 'free':
                n_to_select_criteria = [sum(max_proba >= 1.0 / start_prob_y.shape[1] + 0.2 / start_prob_y.shape[1])]
                ...
                if n_to_select == max_proba.shape[0]:
                    ...
                    get_last_hidden_layer_output = K.function([self.base_estimator_.layers[0].input],
                                                              [self.base_estimator_.layers[-2].output])
                    embedding = get_last_hidden_layer_output(X[safe_mask(X, ~has_label)][selected_tmp])[0]
                    S_selected = embedding @ embedding.transpose()
                    D_selected = np.array(1 - S_selected / S_selected.max())
                    clustering = AgglomerativeClustering(n_clusters=self.k_best, linkage='single',
                                                         affinity='precomputed').fit(D_selected)
                    selected = \
                        [
                ...
                k_per_class = int(np.rint(float(self.k_best) / float(start_prob_y.shape[1])))
                min_n_to_select_list = []
                for class_id in range(start_prob_y.shape[1]):
                    ids_of_labels = np.arange(len(pred))[pred == class_id]
                    if self.threshold is not None and self.threshold > 1:
                        probs_of_class = self.base_estimator_.predict(X[safe_mask(X, has_label)])[:,class_id]
                        threshold_ = np.percentile(probs_of_class, self.threshold)
                    if self.k_best is not None:
                        if self.diverse is None:
                            n_to_select_criteria_for_label = [k_per_class, len(ids_of_labels), sum(
                                max_proba[ids_of_labels] >= 1.0 / start_prob_y.shape[1] + 0.2 / start_prob_y.shape[1])]
                        else:
                            n_to_select_criteria_for_label = [k_per_class * self.diverse, len(ids_of_labels), sum(
                                max_proba[ids_of_labels] >= 1.0 / start_prob_y.shape[1] + 0.2 / start_prob_y.shape[1])]
                    else:
                ...
                for class_id in range(start_prob_y.shape[1]):
                    ...
                    if n_to_select == max_proba[ids_of_labels].shape[0]:
                        ...
                        S_selected = embedding @ embedding.transpose()
                        D_selected = np.array(1 - S_selected / S_selected.max())
                        clustering = AgglomerativeClustering(n_clusters=k_per_class, linkage='single',
                                                             affinity='precomputed').fit(D_selected)
                        selected = selected + \
                                   [
                                       selected_tmp[c_id == clustering.labels_]
                ...
                for class_id in range(start_prob_y.shape[1]):
                    ...
                    if self.threshold is not None and self.threshold > 1:
                        ...
                        threshold_ = np.percentile(probs_of_class, self.threshold)
                    if self.k_best is not None:
                        k_per_class = int(np.rint(float(self.k_best) * self.ratio_dict[output_encoder.categories_[0][class_id]]))
                        if self.diverse is None:
                            n_to_select_criteria_for_label = [k_per_class, len(ids_of_labels), sum(
                                max_proba[ids_of_labels] >= 1.0 / start_prob_y.shape[1] + 0.2 / start_prob_y.shape[1])]
                        else:
                    ...
                    if n_to_select_for_label == max_proba[ids_of_labels].shape[0]:
                        ...
                        selected_tmp_ids = np.argpartition(-max_proba[ids_of_labels], n_to_select_for_label)[
                                           :n_to_select_for_label]
                        selected_tmp = ids_of_labels[selected_tmp_ids]
                        get_last_hidden_layer_output = K.function([self.base_estimator_.layers[0].input],
                                                                  [self.base_estimator_.layers[-2].output])
                        embedding = get_last_hidden_layer_output(X[safe_mask(X, ~has_label)][selected_tmp])[0]
                        S_selected = embedding @ embedding.transpose()
                        #D_selected = np.array(1 - S_selected / S_selected.max())
                        D_selected = np.nan_to_num(np.array(1 - S_selected / S_selected.max()))
                        clustering = AgglomerativeClustering(n_clusters=k_per_class, linkage='single',
                                                             affinity='precomputed').fit(D_selected)
                        #clustering = AgglomerativeClustering(n_clusters=k_per_class, linkage='single',
                        #                                     affinity='euclidean').fit(embedding)
                        selected = selected + \
                                   [
                                       selected_tmp[c_id == clustering.labels_]
                                       [
                        ...
                        selected_label_ids = np.argpartition(-max_proba[ids_of_labels], n_to_select_for_label)[
                                             :n_to_select_for_label]
                        selected = selected + list(ids_of_labels[selected_label_ids])
            ...
            self.transduction_[selected_full] = np.vectorize(label_dict.get)(pred[selected])
            ...
            self.base_estimator_ = clone_model(self.base_estimator_)
            # self.base_estimator_.compile(loss=CategoricalCrossentropy(), optimizer='adam', metrics=['accuracy'])
            self.base_estimator_.compile(loss=lossfn, optimizer='adam', metrics=['accuracy'])
    ...
    def predict(self, X):
        ...
        return np.argmax(self.base_estimator_.predict(X), axis=-1)

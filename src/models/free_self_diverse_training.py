        ...
        while not np.all(has_label) and (
            ...
            if self.balance == 'free':
                ...
                if n_to_select == max_proba.shape[0]:
                    ...
                    D = np.zeros((n_to_select, n_to_select))
                    try:
                        leaves = self.base_estimator_.predict(X[safe_mask(X, ~has_label)][selected_tmp], pred_leaf=True, n_jobs=-1)
                    except:
                        leaves = self.base_estimator_.apply(X[safe_mask(X, ~has_label)][selected_tmp])
                    try:
                        selected_onehot = OneHotEncoder().fit_transform(leaves)
                    except:
                        selected_onehot = OneHotEncoder().fit_transform(leaves.reshape(-1, 1))
                    S_selected = (selected_onehot * selected_onehot.transpose()).todense()
                ...
                for label in self.base_estimator_.classes_:
                    ...
                    if n_to_select == max_proba[ids_of_labels].shape[0]:
                        ...
                        D = np.zeros((n_to_select, n_to_select))
                        try:
                            leaves = self.base_estimator_.predict(X[safe_mask(X, ~has_label)][selected_tmp],
                                                                  pred_leaf=True, n_jobs=-1)
                        except:
                            leaves = self.base_estimator_.apply(X[safe_mask(X, ~has_label)][selected_tmp])
                        try:
                            selected_onehot = OneHotEncoder().fit_transform(leaves)
                ...
                for label in self.base_estimator_.classes_:
                    ...
                    if n_to_select_for_label == max_proba[ids_of_labels].shape[0]:
                        ...
                        selected_tmp = ids_of_labels[selected_tmp_ids]
                        D = np.zeros((n_to_select_for_label, n_to_select_for_label))
                        try:
                            leaves = self.base_estimator_.predict(X[safe_mask(X, ~has_label)][selected_tmp],
                                                                  pred_leaf=True, n_jobs=-1)
                        except:
